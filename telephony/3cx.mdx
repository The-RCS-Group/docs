...
<AccordionGroup>
  ...
  <Accordion title="Inbound calls ring but the AI doesn't answer">
    The DID is likely not added to the SmartAlex trunk's accepted numbers list. In 3CX, check the trunk's **DIDs** tab includes the number being dialled. In SmartAlex, the DID must be registered on the inbound side (provisioned automatically by Support if you're on Option A with non-SmartAlex DIDs).
  </Accordion>
