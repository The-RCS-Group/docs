...
| Retention period | 90 days | Per tenant, 7 to 365 days |
| Recording format | MP3, 64 kbps | No |
| Access | Tenant admins and agents with the correct role permission | RBAC gated |
| Deletion on retention expiry | Automatic | No override |
| Download | Admin users only | RBAC gated |
...
All resources in SmartAlex are scoped to a tenant (organisation). Row-level security enforced at the database layer ensures no cross-tenant data access, regardless of application-level bugs.

Within a tenant, role-based access control gates:
...
- Configuring RBAC correctly for their team
