...
<AccordionGroup>
  ...
  <Accordion title="Trunk registered but inbound calls fail">
    ...
    - Wallet empty (test-mode tenants). Top up in Settings → Billing.
  </Accordion>
  <Accordion title="AI answers but hangs up immediately">
    ...
    - The call exceeded the tenant's max-call-duration (default 300s).
    - The LiveKit dispatch failed , check the call_records table or contact support.
  </Accordion>
  ...
  <Accordion title="Calls drop at exactly 5 minutes">
    The tenant's `max_duration` is set to 300 seconds. Increase it in the agent settings if longer calls are expected.
  </Accordion>
