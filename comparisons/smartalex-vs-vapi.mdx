...
The platform is built on a multi-tenant architecture, allowing agencies, franchises, and multi-location businesses to manage separate client environments from a single account. SmartAlex also provides an MCP (Model Context Protocol) server with 28 tools, enabling AI assistants to interact with the platform through natural language commands.
...
| **Workspace Isolation** | Native workspace separation with role-based permissions | Single API key per account |
...
| **Voice Options** | 50+ curated voices with emotion control | Multiple providers, highly configurable |
| **AI Models** | Curated language model stack, configurable per agent | Any LLM (OpenAI, Anthropic, Groq, custom) |
| **Speech-to-Text** | Curated, high-accuracy across accents | Deepgram, Azure, Google, Whisper, custom |
| **Text-to-Speech** | Ultra-low-latency synthesis with SSML support | ElevenLabs, Play.ht, Azure, Cartesia, custom |
...
| **Webhook Events** | Wide range of event types | Comprehensive server-side events |
| **Mobile App** | React Native (iOS + Android) | No mobile app |
| **Telephony** | Managed infrastructure | BYO or managed carrier/SIP |
| **Enterprise Auth** | Role-based access with SSO/SAML on Enterprise | API key, OAuth available |
| **Messaging** | Telegram bot, WhatsApp Business | Not available |
| **Email** | Gmail and Outlook for inbound/outbound, plus dedicated outreach | Not included |
...
- **Language model**: Curated stack tuned for natural conversation, configurable per agent with system prompt customisation and context window management
- **Text-to-Speech**: Ultra-low-latency synthesis with emotion control, speed adjustment (0.6x–1.5x), and volume control (0.5x–2.0x)
...
- **Webhook System** with a wide range of event types for real-time event processing and automation
...
- **Messaging**: Telegram bot for conversational platform management, plus WhatsApp Business
- **Real Estate**: Property developer APIs, Dubai Land Department data integration, multi-developer architecture
- **Security**: Device fingerprinting, geographic verification via IP-to-country mapping, multi-signal threat scoring
...
| Confirmation | Email via Gmail/Outlook | Custom email integration |
...
- **Role-based access control** per workspace (Owner / Admin / Member / Viewer)
- **Workspace-level data isolation** ensuring data never crosses customer boundaries
...
- Want built-in security monitoring (device fingerprinting, threat scoring, geographic call permissions to prevent international fraud)
...
2. **Function calling / Tools**: Map VAPI function calling configurations to SmartAlex's built-in agent tools (transfer, calendar, lead capture, SMS, email, custom HTTP via Power Tools). Custom integrations may need to be rebuilt against SmartAlex's REST API or webhook system.
...
6. **Webhooks**: Map VAPI webhook handlers to SmartAlex's webhook event system.
