...
The platform uses a multi-tenant architecture, meaning agencies and multi-location businesses can manage separate client environments from one account. SmartAlex also offers an MCP (Model Context Protocol) server with 28 tools, enabling AI assistants like Claude and ChatGPT to interact with the platform directly through natural language.
...
| **Voice Agent Builder** | Visual builder with Standard (URL-based) and Advanced wizard, plus Squad Architect for multi-agent setups | API-based configuration |
...
| **Workspace Isolation** | Native workspace separation with role-based permissions | Single account per API key |
...
| **AI Models** | Curated language model stack, configurable per agent | GPT-4, Claude, custom LLMs |
| **A/B Testing** | Agent mode comparison | Built-in A/B testing framework |
| **MCP Server** | 28-tool MCP server for AI assistant integration | GPT App for ChatGPT |
...
| **Webhook Support** | Wide range of webhook event types | Comprehensive webhook system |
| **Mobile App** | React Native app (iOS + Android) | No mobile app |
| **SSO / Enterprise Auth** | Role-based access with SSO/SAML on Enterprise | API key authentication |
| **Compliance** | Geographic permissions, fraud prevention, encryption in transit and at rest | SOC 2 compliant |
| **Messaging Integration** | Telegram bot, WhatsApp Business | Not included |
| **Email Integration** | Gmail and Outlook for inbound/outbound, dedicated outreach campaigns | Not included |
| **Security** | Device fingerprinting, geographic verification, multi-signal threat scoring | Standard API security |
...
| Plan | Price | Includes |
|------|-------|----------|
| **Professional** | $99/mo | 250 included minutes, 1 phone number, 1 agent, 10 concurrent slots, knowledge base, calendar & email integrations |
| **Dental** | $349/mo | 300 included minutes, pre-tuned for dental practices |
| **Real Estate** | $3,000/mo | 300 included minutes, 10 phone numbers, lead tracking & CRM, showing bookings, dedicated account manager |
| **Enterprise** | Custom | Volume pricing, SSO/SAML, custom integrations, 99.9% uptime SLA, white-label options |

Additional voice minutes are billed at $0.08/min web and $0.11/min phone. All plans include the web dashboard, agent builder, and contact management.
...
SmartAlex uses a curated voice stack to deliver high-quality, natural-sounding conversations:

- **50+ voices** curated for business use across multiple languages and accents
- **Ultra-low-latency voice synthesis** with emotion control, speed (0.6x–1.5x), and volume control (0.5x–2.0x)
- **High-accuracy speech recognition** across accents and languages
- **Curated language model stack** tuned for natural conversation, with per-agent customisation
- **Emotion-aware responses** with SSML support for natural prosody
- **Sub-second latency** for real-time conversational flow
- **Two creation paths** (Standard URL-based and Advanced wizard) plus Squad Architect for multi-agent designs
...
- **Webhook system** with a wide range of event types for real-time notifications and event-driven architectures
...
- **AI Assistants**: MCP server for Claude, ChatGPT, and other AI tools
...
- **Workspace-level data isolation** ensuring client data never crosses boundaries
- **Encrypted secrets management** for credentials and API keys
...
1. **Agent configurations**: Agent prompts, tool definitions, and voice settings can be recreated in SmartAlex's agent builder. The Standard (URL-based auto-build) and Advanced wizard accelerate setup.
...
5. **Integrations**: Any custom integrations built with Retell's function calling would need to be rebuilt using SmartAlex's webhook system or REST API.

SmartAlex provides migration assistance for Real Estate and Enterprise plan customers.
...
SmartAlex uses a curated language model stack and supports configuring models per agent. While it does not currently support fully custom LLM endpoints via WebSocket (as Retell does), the platform's agent system is designed to work optimally with the supported models. For most business use cases, the built-in model options provide excellent performance without the overhead of managing custom LLM infrastructure.
</Accordion>

<Accordion title="How does pricing compare for 1,000 minutes per month?">
At 1,000 minutes per month, Retell AI would cost approximately $70-$150 depending on configuration (at $0.07-$0.15/min), plus any additional telephony and LLM costs. SmartAlex's Professional plan starts at $99/month with 250 included minutes — overage minutes are charged at $0.08-$0.11/min, and the plan bundles CRM, campaign tools, and analytics that you would otherwise need to purchase separately. The total cost of ownership favors SmartAlex when you factor in the CRM, campaign tools, and analytics you would otherwise need to purchase separately.
</Accordion>

<Accordion title="Which platform has better call quality and lower latency?">
Both platforms deliver high-quality voice experiences. SmartAlex uses a curated low-latency voice synthesis and speech-recognition stack to achieve sub-second response latency. Retell AI offers similar quality with the added benefit of dynamic voice speed adjustment and PCAP debugging for troubleshooting. In practice, end-user call quality is comparable between the two platforms, with differences mainly in fine-tuning capabilities (Retell) versus ease of configuration (SmartAlex).
</Accordion>
...
SmartAlex supports comparing agent configurations side-by-side, but does not have a dedicated A/B testing framework like Retell. If systematic A/B testing of voice configurations is a core requirement, Retell's built-in testing framework is more mature. For most businesses, SmartAlex's analytics provide sufficient insight into agent performance without formal A/B testing infrastructure.
</Accordion>
