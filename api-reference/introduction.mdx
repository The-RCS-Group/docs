...
POST and PATCH requests accept JSON bodies:
...
API requests are rate limited to prevent abuse and ensure platform stability. Rate limits are enforced per API key. Rate limit headers are included in every response:
